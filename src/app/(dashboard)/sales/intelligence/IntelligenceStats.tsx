...
export function IntelligenceStats() {
  const { data, isLoading } = useIntelligenceStats();
  ...
  return (
    <section className={styles.statsGrid}>
      <StatsCard label="Calls Processed" value={data.calls_processed} icon={<PhoneIcon />} />
      <StatsCard
        label="Proposals Generated"
        value={`${data.proposals_generated_pct.toFixed(0)}%`}
        sublabel={`${data.proposals_generated} total`}
        icon={<DocIcon />}
      />
      <StatsCard
        label="Flagged / Blocked"
        value={`${data.flagged_pct.toFixed(0)}%`}
        sublabel={`${data.flagged_count} calls`}
        icon={<BlockIcon />}
      />
      <StatsCard
        label="Awaiting Review"
        value={data.awaiting_review}
        sublabel={`${data.awaiting_review_pct.toFixed(0)}% of processed`}
        icon={<HourglassIcon />}
      />
    </section>
  );
