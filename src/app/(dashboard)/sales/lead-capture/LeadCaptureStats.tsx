...
export function LeadCaptureStats() {
  const { data, isLoading } = useLeadStats();

  if (isLoading) return <StatsSkeleton />;
  if (!data) return null;

  const { by_status } = data;

  return (
    <>
      <StatsCard label="New" value={by_status.new ?? 0} icon={<NewIcon />} />
      <StatsCard label="Contacted" value={by_status.contacted ?? 0} icon={<ContactedIcon />} />
      <StatsCard label="Proposal" value={by_status.proposal ?? 0} icon={<ProposalIcon />} />
      <StatsCard label="Onboarding" value={by_status.onboarding ?? 0} icon={<OnboardingIcon />} />
      <StatsCard label="Canceled" value={by_status.canceled ?? 0} icon={<CanceledIcon />} />
    </>
  );
}
